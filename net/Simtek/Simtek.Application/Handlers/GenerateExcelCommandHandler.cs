...
public class GenerateExcelCommandHandler(IInterventionRepository interventionRepository):IRequestHandler<GenerateExcelCommand, OperationResult<Stream>>
{
    public async Task<OperationResult<Stream>> Handle(GenerateExcelCommand request, CancellationToken cancellationToken)
    {
        try
        {
            var intervention = await interventionRepository.GetInterventionAsync(request.InterventionId,cancellationToken);
            ...
            foreach (var description in descriptions)
            {
                ...
                worksheet.Cells[$"A{descriptionRow}:D{descriptionRow}"].Style.Border.Right.Style = ExcelBorderStyle.Thin;
                descriptionRow++;
            }
            
            // Materials
            var materialsRow = descriptionRow + 5;
            var materials = intervention.Materials;
            ...
            foreach (var material in materials)
            {
                ...
                worksheet.Cells[$"A{materialsRow}:D{materialsRow}"].Style.Border.Right.Style = ExcelBorderStyle.Thin;
